...
import top.aceofspades.core.annotation.Log;
...
public class LogOperationSource {
    ...
    public Collection<LogOperation> getLogOperations(Method method, Class<?> targetClass) {
        ...
        if (cached != null) {
            ...
            Collection<LogOperation> logOps = computeCacheOperations(method, targetClass);
            if (logOps != null) {
    ...
    @Nullable
    private Collection<LogOperation> computeCacheOperations(Method method, Class<?> targetClass) {
        // 不允许非public方法记录日志
